...
import { ArrowUpRight, Github, Mail, Shield, MapPin, Heart } from 'lucide-react';

export default function Footer({ onNavigate, onOpenContact }) {
  return (
    <footer className="bg-slate-950 border-t border-slate-800/80 text-slate-400 py-16 text-xs">
      <div className="container-custom space-y-12">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          {/* Brand Info */}
          <div className="md:col-span-4 space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-teal-400 to-teal-700 flex items-center justify-center font-display font-extrabold text-slate-950 text-sm">
                Q
              </div>
            ...
            <div className="flex items-center gap-2 text-teal-400 font-mono text-[11px]">
              ...
              <span>Quelp V1 Beta Build in Active Development</span>
            </div>
          </div>
          ...
          <div className="md:col-span-2 space-y-3">
            <h4 className="font-display font-bold text-white uppercase text-xs tracking-wider">Engineering</h4>
            <ul className="space-y-2">
              <li><button onClick={() => { onNavigate('home'); setTimeout(() => document.getElementById('architecture')?.scrollIntoView({ behavior: 'smooth' }), 150); }} className="hover:text-white transition-colors cursor-pointer">Architecture</button></li>
              <li><button onClick={() => { onNavigate('home'); setTimeout(() => document.getElementById('roadmap')?.scrollIntoView({ behavior: 'smooth' }), 150); }} className="hover:text-white transition-colors cursor-pointer">Product Roadmap</button></li>
              <li><button onClick={() => { onNavigate('home'); setTimeout(() => document.getElementById('beliefs')?.scrollIntoView({ behavior: 'smooth' }), 150); }} className="hover:text-white transition-colors cursor-pointer">Core Values</button></li>
            </ul>
          </div>
          ...
          <div className="md:col-span-4 space-y-3">
            ...
            <p className="text-slate-400">
              Inquire about pilot program access, custom white-label software, or partnerships.
            </p>
            <div className="pt-2">
              <button
